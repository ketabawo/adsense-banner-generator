...
- AI Creative Assistant：選択中Variantのコピー修正案・変更前後比較・確認後のエディタ適用（自動テスト済み、ユーザー操作で修正案取得・適用・折り返し表示を確認済み）
...
### AI相談によるCreativeコピーの修正（2026-09-14）

studio制作の「サイズ別の画像」とエディタの間に「AIとコピーを改善」を追加しました。ログイン後に「メインコピーを短くして」「CTAを分かりやすくして」などの希望を送ると、選択中サイズのメインコピー・サブコピー・CTAについて、修正理由と変更前後を表示します。「この修正案を適用」で既存のCreativeStateの文言だけを更新し、エディタとプレビューへ反映します。非表示のサブコピー・CTAは変更せず、レイアウト・背景・他サイズも保持します。適用後は従来のCampaign下書き保存・Creativeライブラリ保存・Review・入稿を使います。

相談後にコピー・表示状態・サイズが変わった場合、古い案の適用を拒否します。サイズ・編集対象・Creative方式の切替、ログアウトで相談画面を破棄し、進行中のブラウザ通信を中止します（既に開始したProvider側の処理・課金の取消を保証するものではありません）。相談内容と修正理由は画面内だけに保持し、過去の相談を次のリクエストへ自動添付しません。追加の相談は、その時点の編集中コピーを基に行います。修正案の取得だけではコピーを変更せず、保存・入稿は利用者の別操作です。

`GET /api/creative-assistant` は接続設定の有無、`POST` は修正案を返します。Googleログイン、変更操作の同一Origin確認、64KiBのリクエスト本文上限、入力とAI回答の検証を行います。OpenAIへ送るのは相談内容・現在の文言（非表示を含む）・表示状態・画像寸法だけです。画像や広告実績は参照しません。既存の `OPENAI_API_KEY` と `OPENAI_MODEL`（未指定時 `gpt-4.1-mini`）を使用します。既存AI相談の通信処理を共通化し、[OpenAI Structured Outputs](https://developers.openai.com/api/docs/guides/structured-outputs)で修正案を受け取り、`store: false`を指定します。DBマイグレーションは不要です。

検証：通常テスト239件成功・DB統合8件スキップ、型チェック0エラー・0警告、Nodeビルド成功。実ページ構成の自動テストで、適用前は元文言を保持すること、適用後はエディタと同一の編集状態を更新すること、別Variantの文言を保持すること、保存・再読み込み後の復元を確認しました。未認証・別Origin・入力上限・不正AI回答・通信失敗・古い案の拒否も検証しました。OpenAI応答はモックで、実モデルへの相談とブラウザでの目視確認は未実施です。

再開位置：複数固定サイズ画像広告のテスト入稿は完了済みです。次は今回追加した「AIとコピーを改善」の実操作確認です。ページを再読み込みし、studio制作の対象サイズを選び、「CTAを分かりやすくして」などを送信します。変更前後を確認して適用し、プレビューの文字の収まりと、Campaign下書き保存・再読み込み後の文言を確認します。今回の変更は未コミット・未pushです。


### AI修正後の文字の折り返し（2026-09-14）

ユーザー共有画面で、AI修正案の取得・変更前後表示・適用を確認しました。300×250のメインコピー「あなたのサービスをもっと広く」が右側で切れる問題がありました。描画処理が明示的な改行だけを扱っていたため、Canvasで実際のフォントの文字幅を測り、メインコピーとサブコピーを表示幅で自動的に折り返すよう修正しました。手動改行・空行は保持し、文言や指定文字サイズは変更しません。プレビュー・PNG出力・Reviewの入稿画像は共通の描画処理を使用します。行数が増えて高さが足りない場合は、従来の表示領域超過警告を表示します。

検証：通常テスト245件成功・DB統合8件スキップ、型チェック0エラー・0警告、Nodeビルド成功。文字幅を模したCanvasテストで報告文言の折り返し、左右・中央揃えの余白、明示改行、サブコピーの高さ超過、Unicode文字の保持を確認しました。修正後の実ブラウザ表示は未確認です。次は現在の文言のままプレビューの折り返しを確認し、その後、別サイズの保持・Campaign下書き保存・再読み込み後の復元の手動確認を続けます。AIへの再相談は不要です。

### 折り返し修正の手動確認完了（2026-09-14）

折り返し修正後、ユーザーから「テストOKです」との報告を受けました。直前に依頼した、現在のコピーでのプレビューの折り返し確認が完了しました。前項の「修正後の実ブラウザ表示は未確認」は解消しました。確認根拠はユーザー報告です。別サイズの保持・保存・再読み込み後の復元・古い案の適用拒否については自動テストで確認済みですが、この報告だけから個別の手動確認完了までは判断していません。
